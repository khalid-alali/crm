...
function SortLabel({ active, dir }: { active: boolean; dir: SortDir }) {
  if (!active) return <span className="text-gray-300 font-normal">↕</span>
  return <span className="text-gray-700 font-normal">{dir === 'asc' ? '↑' : '↓'}</span>
}
    ...
    return (
      <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wide">
        <button
          ...
          className="inline-flex items-center gap-1.5 hover:text-gray-800 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 rounded"
        >
          {label}
              ...
              return (
                <tr key={owner.id} className="hover:bg-gray-50">
                  <td className="px-4 py-2.5">
                    <Link href={`/owners/${owner.id}`} className="font-medium text-blue-600 hover:underline">
                      {owner.name}
                    </Link>
                  </td>
                  <td className="px-4 py-2.5 text-gray-600">{owner.email?.trim() ? owner.email : '—'}</td>
                  <td className="px-4 py-2.5 text-gray-600 tabular-nums">{owner.location_count}</td>
                  <td className="px-4 py-2.5 text-gray-600">
                    {display && tel ? (
                      <a href={tel} className="text-blue-600 hover:underline tabular-nums">
                        {display}
                      </a>
                    ) : (
                      '—'
