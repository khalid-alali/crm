...
export default function TeslaEnrollSearchModal({
  open,
  onClose,
  enrolledLocationIds,
  enrollingLocationId,
  ...
  useEffect(() => {
    ...
    const url = `/search?q=${encodeURIComponent(debouncedQuery)}&context=tesla-enroll`
  ...
  const showNoResults = hasTypedQuery && !loading && results.shops.length + results.contacts.length + results.accounts.length === 0
  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="tesla-enroll-modal-title"
      ...
      >
        <div className="border-b border-arctic-200 px-4 py-3">
          <h2 id="tesla-enroll-modal-title" className="text-base font-semibold text-onix-900">
            Enroll shop in Tesla
          </h2>
          <p className="mt-1 text-sm text-onix-500">Search like global search, then enroll the location.</p>
          {errorMessage ? (
        ...
        <div className="flex items-center gap-2 border-b border-arctic-200 px-4 py-3">
          ...
          <input
            ref={inputRef}
            id="tesla-enroll-search"
            name="tesla_enroll_search_query"
            type="search"
            inputMode="search"
            enterKeyHint="search"
            aria-autocomplete="list"
            aria-controls="tesla-enroll-results"
            {...crmInputNoAutofillProps}
            value={query}
            onChange={e => setQuery(e.target.value)}
            placeholder="Shop name, person, business, admin shop id…"
            className="min-w-0 flex-1 bg-transparent text-base text-onix-900 placeholder:text-onix-400 focus:outline-none"
          />
          <kbd className="hidden shrink-0 rounded border border-arctic-300 bg-arctic-50 px-1.5 py-0.5 text-xs text-onix-500 sm:inline">
            esc
          </kbd>
        </div>

        <div id="tesla-enroll-results" className="max-h-[min(420px,55vh)] overflow-y-auto">
          {loading && (
          ...
          {showNoResults && (
            <div className="border-t border-arctic-100 px-4 py-10 text-center">
              ...
              <p className="mt-1 text-xs text-onix-500">Try another name, email, or admin shop id.</p>
            </div>
          )}
        </div>
